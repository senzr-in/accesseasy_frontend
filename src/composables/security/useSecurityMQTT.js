...
function handleFrigateEvent(_topic, payload) {
  ...
  catch { console.warn('[useSecurityMQTT] Could not parse frigate/events payload'); return; }

  const { type, after } = msg;
  if (!after) return;

  const label = after.label;
  console.debug(`[useSecurityMQTT] ▶ frigate/events  type=${type}  label=${label}  camera=${after.camera}`);
