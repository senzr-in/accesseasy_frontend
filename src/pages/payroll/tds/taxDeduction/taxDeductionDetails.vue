...
const handleRowClick = (item) => {
  ...
  showForm.value = true;
  router.push(`/payroll/tds/deduction/${item.id}`);
};
...
const fetchtds = async () => {
  ...
  try {
    ...
    const personalItems = personalData.data;
    const employeeIds = personalItems.map((item) => item.employeeId).filter(Boolean);

    let enrichedItems = [...personalItems];

    if (employeeIds.length > 0) {
      const tdsResults = await fetchTdsDeductionByEmployees(employeeIds);

      // merge before setting to `items.value`
      enrichedItems = personalItems.map((item) => {
        const matchingTds = tdsResults.find(
          (tdsItem) =>
            String(tdsItem.employer?.employeeId) === String(item.employeeId)
        );
        ...
        return {
          ...
          totalEarnings: matchingTds?.totalEarnings || "Not Finalized",
          totalDeductions: matchingTds?.totalDeductions || 0,
          taxableIncome: matchingTds?.taxableIncome || 0,
          exemption: matchingTds?.exemption || 0,
          taxCharge: matchingTds?.paidTax || 0,
          standardDeduction: matchingTds?.standardDeduction || 0,
          regime: matchingTds?.regime || 0,
          status: matchingTds?.status || 0,
          id: matchingTds?.id || 0,
        };
      });
    }
    ...
    showError.value = true;
    errorMessage.value = error.message || "Failed to fetch logs. Please try again.";
  } finally {
    loading.value = false;
  }
};
...
if (filters.financialYear) {
  ...
  for (let i = 0; i < employeeIds.length; i += BATCH_SIZE) {
    ...
    const params = {
      ...
      "filter[_and][0][_and][3][employer][employeeId][_in]": batch.join(","),
