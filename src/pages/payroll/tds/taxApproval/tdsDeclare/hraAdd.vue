...
const processEarningsAndDeductions = (data) => {
  ...
  rentEntries.value = tds?.hraRent?.hraRent || [];
...
const save = async () => {
  ...
  try {
    let updatedRents = JSON.parse(JSON.stringify(rentEntries.value)) || [];

    const newEntry = {
      id: editedItem.id || (updatedRents.length ? Math.max(...updatedRents.map(i => i.id || 0)) + 1 : 1),
    ...
    const existingIndex = updatedRents.findIndex((item) => item.id === editedItem.id);
    if (existingIndex !== -1) {
  ...
  if (item.fileId) {
    try {
      ...
      if (response.ok) {
        ...
        if (!fileInput) {
          console.warn("⚠️ File input element not found — likely not rendered or inside a v-if.");
        } else {
          const dataTransfer = new DataTransfer(); // ✅ Declare before use
const fakeFile = new File(["placeholder content"], fileMeta.filename_download, {
...
editedFile.value = fakeFile;
          ...
          try {
            ...
            console.log("✅ fileInput.files set successfully:", fileInput.files);
          } catch (err) {
            console.error("❌ Error while setting fileInput.files:", err);
          }
     if (fileInput.files.length === 0) {
            console.warn("⚠️ fileInput.files is still empty — browser may block programmatic assignment.");
          } else {
            console.log("🎉 File successfully shown in file input");
          }
        }
      } else {
        console.error("❌ Failed to fetch file metadata. HTTP Status:", response.status);
      }
    } catch (err) {
