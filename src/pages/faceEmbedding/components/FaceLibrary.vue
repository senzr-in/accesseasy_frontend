  ...
  <div class="h-full w-full bg-[#F8FAFC] dark:bg-[#0b0f19] text-slate-800 dark:text-slate-100 p-6 overflow-y-auto selection:bg-indigo-500/20 font-sans">
    ...
    <div v-if="activeTab === 'unknown'" class="space-y-4">
      <div class="bg-white dark:bg-[#151c2c] border border-slate-200/80 dark:border-white/10 rounded-2xl p-4 shadow-sm">
        <div class="flex items-center gap-3">
          <div class="p-2.5 bg-purple-50 dark:bg-purple-500/20 text-purple-600 rounded-xl text-xl">🛡️</div>
          <div>
            <h3 class="text-sm font-bold text-slate-900 dark:text-white">Unrecognized Subject Audits</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">Captured face crops that require enrollment or identity classification.</p>
          </div>
        </div>
      </div>

      <div v-if="unknownFaces.length === 0" class="text-center py-12 text-slate-400 text-xs italic">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        >
          <div class="relative aspect-video rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/10">
            <img :src="face.snapshot" alt="Unknown" class="w-full h-full object-cover" />
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <div v-if="filteredPeople.length === 0" class="col-span-full text-center py-12 text-slate-400 text-xs italic">
      ...
      >
        <div class="space-y-4">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">{{ person.department }}</p>
          </div>
          ...
          <div class="space-y-1.5 pt-2 border-t border-slate-100 dark:border-white/10 text-xs text-slate-500 dark:text-slate-400">
            <div class="flex justify-between items-center">
              ...
              <span v-if="person.multiPoseEnrolled" class="text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 px-2 py-0.5 rounded-md font-mono font-bold text-[11px]">
      ...
      <div class="bg-white dark:bg-[#151c2c] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden text-slate-900 dark:text-white font-sans">
        ...
        <div class="flex items-center gap-0 px-6 pt-4 cursor-pointer">
          ...
          >
            ...
            <div v-if="i < 2" class="flex-1 mx-2">
              <div :class="['h-0.5 rounded-full transition', i < stageIndex ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-white/10']"></div>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <!-- ── STAGE 1: Details ── -->
          <div v-if="wizardStage === 'details'" class="space-y-4">
            ...
            <div class="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-4">
              ...
              <div class="flex gap-2 justify-between">
                <div v-for="(p, i) in POSES" :key="i" class="flex flex-col items-center gap-1.5 flex-1">
          ...
          <div v-else-if="wizardStage === 'capture'" class="space-y-3">
            <!-- Progress -->
            ...
            <div class="flex gap-4 items-start">
              ...
              <div class="flex flex-col items-center gap-3 w-40 shrink-0">
                <div class="bg-gradient-to-b from-indigo-50 to-slate-50 dark:from-white/5 dark:to-white/5 rounded-2xl border border-indigo-100 dark:border-white/10 p-3 w-full flex flex-col items-center gap-2">
                  ...
                  <p class="text-[11px] font-bold text-slate-800 dark:text-slate-200 text-center">{{ currentPose?.instruction }}</p>
                  <p class="text-[10px] text-slate-500 text-center leading-snug">{{ currentPose?.tip }}</p>
                </div>
              ...
              <div class="flex-1 flex flex-col gap-3">
                <div class="relative aspect-video rounded-2xl bg-slate-900 border border-slate-700 overflow-hidden shadow-inner flex items-center justify-center">
                  ...
                  <div class="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                    <div class="w-36 h-44 rounded-full border-2 border-dashed border-indigo-400 animate-pulse"></div>
                  </div>

                  <!-- Corner alignment brackets -->
                  <div class="absolute top-2 left-2 w-4 h-4 border-t-2 border-l-2 border-white/80 rounded-sm z-10"></div>
                  <div class="absolute top-2 right-2 w-4 h-4 border-t-2 border-r-2 border-white/80 rounded-sm z-10"></div>
                  <div class="absolute bottom-2 left-2 w-4 h-4 border-b-2 border-l-2 border-white/80 rounded-sm z-10"></div>
                  <div class="absolute bottom-2 right-2 w-4 h-4 border-b-2 border-r-2 border-white/80 rounded-sm z-10"></div>
                  ...
                  <!-- Error Overlay -->
                  <div v-if="webcamError" class="absolute inset-0 bg-slate-900/95 flex flex-col items-center justify-center p-4 text-center z-20">
                    <span class="text-2xl mb-2">⚠️</span>
                    <p class="text-xs font-bold text-rose-400">{{ webcamError }}</p>
                    <button @click="startWebcam" class="mt-3 px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-bold">
                ...
                <div class="flex items-center justify-between text-xs pt-1">
                  <button @click="currentPoseIndex === 0 ? (wizardStage = 'details') : selectPoseIndex(currentPoseIndex - 1)" class="text-slate-500 hover:text-slate-700">
                    &larr; {{ currentPoseIndex === 0 ? 'Back to Details' : 'Previous Pose' }}
                  </button>
                  <button v-if="capturedCount === POSES.length" @click="goToReview" class="px-4 py-1.5 bg-emerald-600 text-white rounded-xl font-bold text-xs">
                    Review All &rarr;
                  </button>
                </div>
              </div>
          ...
          <div v-else-if="wizardStage === 'review'" class="space-y-5">
            <div class="bg-gradient-to-r from-indigo-50 to-emerald-50 dark:from-white/5 dark:to-white/5 rounded-2xl p-4 border border-indigo-100 dark:border-white/10 flex items-center gap-4">
              <img v-if="capturedPoses[0]" :src="capturedPoses[0]" class="w-16 h-16 rounded-2xl object-cover border-2 border-indigo-300 shadow" />
              <div class="flex-1">
                <h3 class="text-base font-bold text-slate-900 dark:text-white">{{ formName }}</h3>
                <p class="text-xs text-slate-500">{{ formDept }} &bull; <span class="capitalize">{{ formType }}</span></p>
                <p v-if="formEmpId" class="text-xs text-indigo-600 font-mono font-bold mt-0.5">{{ formEmpId }}</p>
              </div>
              <div class="text-right">
                <span class="text-3xl font-black text-emerald-600">{{ capturedPoses.filter(Boolean).length }}</span>
                <p class="text-[10px] text-slate-500 font-medium">real poses ready</p>
              </div>
            </div>

            <div>
              <p class="text-xs font-bold text-slate-700 dark:text-slate-300 mb-3">Captured Real Face Pose Samples</p>
              <div class="grid grid-cols-5 gap-3">
                <div v-for="(p, i) in POSES" :key="i" class="flex flex-col items-center gap-1.5">
                  <div class="relative w-full aspect-square rounded-xl overflow-hidden border-2 border-slate-200 dark:border-white/10 bg-slate-100 shadow-sm group">
                    <img v-if="capturedPoses[i]" :src="capturedPoses[i]" class="w-full h-full object-cover" />
                    <button
            ...
            <div class="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl p-3.5 flex items-start gap-2.5 text-xs text-emerald-900 dark:text-emerald-300">
              ...
              <div>
                <p class="font-bold">Quality Gate Passed & AI Vector Sync Ready</p>
            ...
            <div class="flex items-center justify-between pt-1 border-t border-slate-100 dark:border-white/10">
              <button @click="wizardStage = 'capture'; startWebcam();" class="text-slate-500 text-xs">&larr; Back to Webcam</button>
              <button @click="handleSubmitEnrollment" :disabled="isSubmitting" class="px-7 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-md shadow-emerald-600/20 transition">
