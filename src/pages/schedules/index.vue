  ...
  <div class="w-full flex flex-col gap-6 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div v-if="filtered.length === 0" class="border-dashed border-2 border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 rounded-2xl">
      <div class="flex flex-col items-center justify-center p-16 text-center space-y-4">
        ...
        <div class="space-y-1">
          <h3 class="font-black text-lg text-slate-900 dark:text-white">No Schedules Found</h3>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left whitespace-nowrap">
          <thead class="bg-slate-50/80 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th class="h-12 px-6 font-black text-[10px] text-slate-500 uppercase tracking-widest">Schedule Name</th>
              <th class="h-12 px-6 font-black text-[10px] text-slate-500 uppercase tracking-widest">Days Active</th>
              <th class="h-12 px-6 font-black text-[10px] text-slate-500 uppercase tracking-widest">Time Window</th>
              <th class="h-12 px-6 font-black text-[10px] text-slate-500 uppercase tracking-widest">Zones Applied</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-slate-900">
            <tr v-for="sch in filtered" :key="sch.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="font-black text-sm text-slate-900 dark:text-white">{{ sch.name }}</p>
                    <p class="text-[10px] font-bold tracking-wider text-slate-400 uppercase">{{ sch.id }}</p>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                ...
                >
                  <span class="w-1.5 h-1.5 rounded-full" :class="sch.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'"></span>
                  {{ sch.status }}
                </div>
              </td>
              <td class="px-6 py-4 text-right">
                ...
                <button @click="handleDelete(sch.id)" class="h-8 w-8 inline-flex items-center justify-center rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors ml-1">
