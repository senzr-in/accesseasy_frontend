<template>
  <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
    <div class="relative w-full max-w-3xl max-h-[90vh] flex flex-col bg-white dark:bg-zinc-950 rounded-[24px] shadow-2xl shadow-indigo-500/10 border border-white/20 dark:border-zinc-800/80 overflow-hidden transform transition-all animate-in zoom-in-95 duration-300">
      ...
      <div class="px-8 py-6 overflow-y-auto flex-1 bg-zinc-50/50 dark:bg-zinc-950/80 custom-scrollbar">
        <form @submit.prevent="handleSubmit" id="door-form" class="space-y-10">
          ...
          <div class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1.5">
                ...
                <input v-model.number="formData.doorNumber" type="number" required min="1" class="w-full h-9 px-3 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-foreground placeholder:text-zinc-500" />
              </div>
              <div class="space-y-1.5">
                <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Door Name <span class="text-red-500">*</span></label>
                <input v-model="formData.doorName" type="text" required placeholder="Main Entrance" class="w-full h-9 px-3 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-foreground placeholder:text-zinc-500" />
              </div>
          ...
          <div class="space-y-5">
            ...
            <div class="space-y-1.5 max-w-sm">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-60 overflow-y-auto p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-inner">
                <label v-for="dept in departments" :key="dept.id" class="flex items-center gap-2 cursor-pointer p-1.5 rounded-md hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                  <input type="checkbox" :value="dept.id" v-model="formData.assignedDepts" class="w-4 h-4 rounded text-blue-600 focus:ring-blue-600 border-zinc-300 dark:border-zinc-700 dark:bg-zinc-900" />
                  <span class="text-xs font-semibold text-slate-700 dark:text-zinc-300 truncate">{{ dept.departmentName || dept.name }}</span>
                </label>
                <div v-if="departments.length === 0" class="col-span-full text-xs text-zinc-400 italic text-center py-4">No departments found.</div>
              </div>
            </div>
          </div>
        </form>
      ...
      <!-- Inline Error Banner -->
      <div v-if="errorMessage" class="mx-8 mb-4 flex items-start gap-3 p-4 rounded-xl border border-red-200 dark:border-red-800/60 bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-400 text-sm font-medium animate-in slide-in-from-top-2 duration-200">
        <AlertTriangle class="w-4 h-4 mt-0.5 shrink-0 text-red-500" />
        <span class="flex-1 break-words">{{ errorMessage }}</span>
        <button @click="errorMessage = ''" class="shrink-0 text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors">
          <X class="w-4 h-4" />
        </button>
      </div>
      ...
      <div class="relative px-8 py-5 border-t border-zinc-100 dark:border-zinc-800/80 bg-white dark:bg-zinc-950 flex justify-end gap-3 z-10">
        ...
        <button type="submit" form="door-form" :disabled="loading" class="group relative px-6 h-10 rounded-xl bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white flex items-center gap-2 text-[13px] font-bold shadow-[0px_1px_2px_0px_rgba(255,255,255,0.5)_inset,0px_4px_6px_-1px_rgba(59,130,246,0.3)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 active:scale-95">
