      ...
      <div class="flex items-center justify-between p-5 border-b border-slate-100 dark:border-zinc-900 bg-slate-50 dark:bg-zinc-900/50">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-2">
        ...
        <button
          class="h-8 w-8 rounded-full hover:bg-slate-200 dark:hover:bg-zinc-800 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-colors"
          @click="close"
        >
          <X class="h-5 w-5" />
        </button>
        ...
        >
          ...
          >
            ...
            <div class="h-32 bg-slate-100 dark:bg-zinc-950 relative overflow-hidden flex items-center justify-center border-b border-slate-200 dark:border-zinc-800">
              ...
              >
              <div
                v-else-if="imageErrors[event.event_id]"
                class="flex flex-col items-center justify-center p-4 text-center bg-red-50 dark:bg-red-950/30 w-full h-full"
              >
                <Activity class="h-5 w-5 text-red-500 mb-1 opacity-50" />
                <p class="text-[9px] font-bold text-red-600 dark:text-red-400 uppercase tracking-widest">
                  Image Unavailable
                </p>
              </div>
              <Camera
              ...
              <div
                v-if="event.score > 0"
                class="absolute top-2 right-2 bg-black/70 backdrop-blur-md rounded-md px-2 py-1 flex items-center gap-1 border border-white/10 shadow-sm"
              >
                <Focus class="h-3 w-3 text-emerald-400" />
                <span class="text-[10px] font-black text-white">{{ Math.round(event.score * 100) }}%</span>
              </div>
              <!-- Camera Badge -->
              <div class="absolute bottom-2 left-2 bg-white dark:bg-slate-900/90 dark:bg-zinc-900/90 backdrop-blur-md rounded-md px-2 py-1 shadow-sm border border-slate-200 dark:border-zinc-700">
