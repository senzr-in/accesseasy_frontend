  ...
  <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300 p-4 w-full">
    <div class="relative w-full max-w-lg flex flex-col bg-white dark:bg-zinc-950 rounded-[24px] shadow-2xl shadow-emerald-500/10 border border-white/20 dark:border-zinc-800/80 overflow-hidden transform transition-all animate-in zoom-in-95 duration-300">
      ...
      <div class="px-8 py-6 overflow-y-auto max-h-[70vh] bg-zinc-50/50 dark:bg-zinc-950/80 custom-scrollbar">
        <form @submit.prevent="handleSubmit" id="access-level-form" class="space-y-6">
          ...
          <div class="space-y-3">
            ...
            <div v-if="fetchingDoors" class="flex justify-center py-6 text-zinc-400">
              <Loader2 class="w-6 h-6 animate-spin" />
            </div>
            <div v-else-if="doors.length === 0" class="text-center py-6 text-sm text-zinc-400 bg-white dark:bg-zinc-950 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800">
              No doors available
            </div>
            <div v-else class="grid gap-2 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar p-1">
              <label 
                v-for="door in doors" 
            ...
            <p class="text-[10px] text-muted-foreground font-medium">Selected doors will be accessible to users assigned to this level.</p>
          </div>
        </form>
      </div>
      ...
      <div class="relative px-8 py-5 border-t border-zinc-100 dark:border-zinc-800/80 bg-white dark:bg-zinc-950 flex justify-end gap-3 z-10 shrink-0">
        ...
        <button type="submit" form="access-level-form" :disabled="loading" class="group relative px-6 h-10 rounded-xl bg-gradient-to-b from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white flex items-center gap-2 text-[13px] font-bold shadow-[0px_1px_2px_0px_rgba(255,255,255,0.5)_inset,0px_4px_6px_-1px_rgba(16,185,129,0.3)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 active:scale-95">
