<template>
  <div class="h-full flex flex-col overflow-hidden">
    <h2 class="text-lg font-black text-slate-900 dark:text-white drop-shadow-sm flex justify-between items-center shrink-0 mb-3">
      ...
      <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest bg-white dark:bg-slate-900/50 dark:bg-slate-800/50 px-3 py-1 rounded-full border border-slate-200 dark:border-white/10">
        {{ socStore.activePatrols.length }} Units
      </span>
    </h2>

    <div
      v-if="socStore.activePatrols.length === 0"
      class="h-32 rounded-[18px] border-2 border-dashed border-slate-200 dark:border-white/10 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/30"
    >
