<template>
  <div class="flex flex-col h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white overflow-hidden">
    ...
    <div class="flex-1 flex flex-col relative overflow-hidden">
      <!-- Patrol Scanner View -->
      <div v-if="activeTab === 'patrol'" class="flex-1 flex flex-col p-6 items-center justify-center animate-in fade-in zoom-in-95 duration-300">
        <div class="w-full max-w-sm flex flex-col items-center">
          ...
          <h2 class="text-2xl font-bold text-center mb-12 text-slate-900 dark:text-white">Warehouse Entrance</h2>
          ...
          <div class="text-center space-y-1 w-full p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 mb-8 shadow-sm">
            <p class="text-[10px] font-bold uppercase tracking-widest text-slate-500">Next Destination</p>
            <p class="text-sm font-bold text-slate-900 dark:text-white">Parking Structure A</p>
          </div>
          
          <div class="flex w-full gap-4">
            <button class="flex-1 py-3 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold uppercase tracking-wider hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-slate-600 dark:text-slate-300 flex justify-center items-center gap-2 shadow-sm">
      ...
      <!-- Gate Log View -->
      <div v-else-if="activeTab === 'gatelog'" class="flex-1 flex flex-col p-6 animate-in fade-in slide-in-from-right-4 duration-300 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
          <!-- Visitor Card -->
          <div v-for="v in 3" :key="v" class="bg-white dark:bg-slate-800 p-4 rounded-xl flex flex-col gap-4 border border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500 transition-colors shadow-sm group">
            <div class="flex items-center gap-4">
              ...
              <div class="flex-1">
                <h3 class="text-base font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Alex Rivera</h3>
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">10:00 AM • Meeting</p>
              </div>
            </div>
    ...
    <div class="h-20 shrink-0 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex px-6 items-center justify-around shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] dark:shadow-none">
      <button 
        ...
        :class="activeTab === 'patrol' ? 'text-blue-600 dark:text-blue-400 transform -translate-y-1' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'"
      >
      ...
      <button 
        ...
        :class="activeTab === 'gatelog' ? 'text-blue-600 dark:text-blue-400 transform -translate-y-1' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'"
      >
        <Users class="w-6 h-6" />
        <span class="text-[10px] font-bold uppercase tracking-wider">Gate Log</span>
        <span class="absolute top-1 right-2 w-2 h-2 bg-emerald-500 rounded-full border border-white dark:border-slate-950"></span>
      </button>
    </div>
  </div>
</template>
