...
const zonesWithStatus = computed(() => {
  ...
  return props.zones.map(z => {
    const zName = (z.zoneName || z.name || '').toLowerCase();
    const zId = String(z.id);
    const zPatrols = props.patrols.filter(p => {
      const matchId = String(p.zoneId || p.zone_id || p.zone) === zId;
      const matchName = zName && (p.zoneName || '').toLowerCase().includes(zName);
