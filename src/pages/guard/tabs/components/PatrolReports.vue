<template>
  <div class="space-y-6">
    ...
    <!-- Tab Navigation -->
    <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-lg w-fit">
      <button
        v-for="tab in tabs"
        :key="tab.value"
    ...
    <!-- ── Analytics Tab ── -->
    <div v-if="activeTab === 'analytics'" class="space-y-6 animate-in">
      <!-- KPI Cards -->
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="stat-card">
          <p class="ae-section-label mb-2">Compliance Rate</p>
          <p class="text-2xl font-bold text-slate-900">94.8%</p>
          <p class="text-xs text-emerald-600 font-semibold mt-1">↑ 1.2% this week</p>
        </div>
        <div class="stat-card">
          <p class="ae-section-label mb-2">Avg Duration</p>
          <p class="text-2xl font-bold text-slate-900">24m 12s</p>
          <p class="text-xs text-rose-600 font-semibold mt-1">↓ 34s slower</p>
        </div>
        <div class="stat-card">
          <p class="ae-section-label mb-2">Total CP Scans</p>
          <p class="text-2xl font-bold text-slate-900">1,402</p>
          <p class="text-xs text-slate-400 mt-1">This month</p>
        </div>
        <div class="stat-card border-l-4 border-l-rose-500">
          <p class="ae-section-label mb-2 text-rose-600">Missed Alerts</p>
          <p class="text-2xl font-bold text-rose-600">14</p>
          <p class="text-xs text-rose-500 font-semibold mt-1">↑ 2 this week</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="ae-card p-5">
          <h3 class="text-sm font-semibold text-slate-900 mb-4">Compliance Rate — This Week</h3>
          <div class="h-36 w-full">
            <svg class="w-full h-full" viewBox="0 0 400 120" preserveAspectRatio="none">
              ...
              <path d="M 10 100 Q 80 40 150 55 T 290 25 T 390 15" fill="none" stroke="#4F46E5" stroke-width="2.5" stroke-linecap="round" />
              <path d="M 10 100 Q 80 40 150 55 T 290 25 T 390 15 L 390 120 L 10 120 Z" fill="#4F46E5" opacity="0.06" />
            </svg>
          </div>
        ...
        <!-- Duration Bar Chart -->
        <div class="ae-card p-5">
          <h3 class="text-sm font-semibold text-slate-900 mb-4">Avg Duration (Minutes)</h3>
          <div class="h-36 flex justify-between items-end gap-2 px-2">
            <div v-for="bar in barChartData" :key="bar.label" class="flex-1 flex flex-col items-center gap-1">
              <div
      ...
      <div class="ae-card p-5 flex items-center justify-between">
        <div>
          <h3 class="text-sm font-semibold text-slate-900">Download Reports</h3>
          <p class="text-xs text-slate-400 mt-0.5">Generate compliance reports and data extracts</p>
        </div>
        <div class="flex gap-2">
          <button class="btn-primary text-xs" @click="exportReport('PDF')">
            <FileDown class="w-3.5 h-3.5" /> PDF Report
          </button>
          <button class="btn-secondary text-xs" @click="exportReport('CSV')">
    ...
    <!-- ── Incident Report Tab ── -->
    <div v-if="activeTab === 'incident'" class="space-y-5 animate-in">

      <!-- Submitted confirmation -->
      <transition name="fade">
        <div v-if="submitted" class="ae-card p-6 border-l-4 border-l-emerald-500 bg-emerald-50">
          <div class="flex items-center gap-3">
            <CheckCircle class="w-5 h-5 text-emerald-600 shrink-0" />
            <div>
              <p class="text-sm font-bold text-emerald-800">Incident Report Submitted</p>
              <p class="text-xs text-emerald-700 mt-0.5">Report #{{ submittedId }} has been logged. Download a PDF copy below.</p>
            </div>
            <button @click="downloadPdf" class="btn-primary text-xs ml-auto">
      ...
      <div class="ae-card p-6">
        <div class="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 class="text-sm font-bold text-slate-900">File Incident Report</h3>
            <p class="text-xs text-slate-400">Guards and admins can file a report at any time.</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="space-y-1.5">
            ...
            <input v-model="incident.guardName" type="text" class="ae-input" placeholder="Auto-filled from session" />
          </div>

          <!-- Date & Time -->
          <div class="space-y-1.5">
            <label class="ae-section-label">Date & Time of Incident</label>
            <input v-model="incident.dateTime" type="datetime-local" class="ae-input" />
          </div>
          ...
          <div class="space-y-1.5">
            <div class="flex justify-between items-end">
              ...
              <button @click="getCurrentLocation" type="button" class="text-[10px] text-indigo-600 font-bold hover:text-indigo-800 flex items-center gap-1 transition-colors" :disabled="gettingLocation">
                <Loader2 v-if="gettingLocation" class="w-3 h-3 animate-spin" />
                <MapPin v-else class="w-3 h-3" />
                {{ gettingLocation ? 'Locating...' : 'Use Current Location' }}
              </button>
            </div>
            <input v-model="incident.location" type="text" class="ae-input" placeholder="e.g. Main Gate, Floor 2 East (or GPS)" />
          </div>

          <!-- Incident Type -->
          <div class="space-y-1.5">
            <label class="ae-section-label">Incident Type</label>
            <select v-model="incident.type" class="ae-select">
          ...
          <div class="space-y-1.5">
            ...
            <div class="flex gap-2">
              <button
                ...
                :class="incident.severity === level.value ? level.activeClass : 'border-slate-200 text-slate-400 hover:border-slate-300'"
              >
                {{ level.label }}
              </button>
            </div>
          ...
          <div class="space-y-1.5">
            ...
            <input v-model="incident.personsInvolved" type="text" class="ae-input" placeholder="Names or descriptions" />
          </div>
          ...
          <div class="md:col-span-2 space-y-1.5 border-t border-slate-100 pt-3">
            <label class="ae-section-label">Attach Photos / Evidence</label>
            <div class="flex items-center justify-center w-full">
              <label class="flex flex-col items-center justify-center w-full h-24 border-2 border-slate-300 border-dashed rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors">
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p class="text-xs text-slate-500"><span class="font-bold text-indigo-600">Click to upload</span> or drag and drop</p>
            ...
            <!-- Upload Previews -->
            <div v-if="incident.localFiles.length" class="flex gap-2 mt-2">
               <div v-for="(file, idx) in incident.localFiles" :key="idx" class="relative w-12 h-12 rounded-lg border border-slate-200 overflow-hidden bg-slate-100 flex items-center justify-center group">
                 <img v-if="file.type.startsWith('image/')" :src="file.preview" class="w-full h-full object-cover" />
                 <span v-else class="text-[10px] font-bold text-slate-500 uppercase">{{ file.name.split('.').pop() }}</span>
                 <button @click="removeFile(idx)" class="absolute inset-0 bg-black/50 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <X class="w-4 h-4" />
                 </button>
               </div>
            </div>
        ...
        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
          <button @click="resetForm" class="btn-ghost">Reset</button>
          <button
      ...
      <div class="ae-card overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
          <h3 class="text-sm font-semibold text-slate-900">Recent Incident Reports</h3>
        </div>
        <table class="ae-table">
          ...
          <tbody>
            <tr v-for="rep in previousReports" :key="rep.id">
              <td><span class="font-mono text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded">{{ rep.id }}</span></td>
              <td class="font-medium text-slate-700">{{ rep.type }}</td>
              <td class="text-slate-500">{{ rep.location }}</td>
              <td>
                <span class="badge" :class="getSeverityClass(rep.severity)">{{ rep.severity }}</span>
              </td>
              <td class="text-slate-700">{{ rep.guardName }}</td>
              <td class="text-slate-500">{{ rep.date }}</td>
              <td>
                <div v-if="rep.attachments && rep.attachments.length" class="flex -space-x-2">
                  <div v-for="fileId in rep.attachments" :key="fileId" 
                       class="w-6 h-6 rounded border-2 border-white bg-slate-200 overflow-hidden shrink-0 shadow-sm"
                       title="View attached photo">
                    <img :src="`http://localhost:8055/assets/${fileId}?fit=cover&width=50&height=50`" class="w-full h-full object-cover" />
                  </div>
                </div>
            ...
            <tr v-if="!previousReports.length">
              <td colspan="6" class="py-10 text-center text-sm text-slate-400">No incident reports filed yet.</td>
            </tr>
          </tbody>
        </table>
      </div>
