<template>
  <div class="flex flex-col h-full bg-white dark:bg-slate-900">
    ...
    <div class="shrink-0 px-5 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
      <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
        Live Patrol Status
        <span
          v-if="activePatrols.length"
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar">
      <!-- Live Patrols Table -->
      <div
        v-if="allPatrols.length"
        class="overflow-x-auto"
      >
        <table class="w-full text-left">
          <thead>
            <tr class="border-b border-slate-100 dark:border-slate-700">
              <th class="px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-400">
                Guard
              </th>
              <th class="px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-400">
                Zone / Checkpoint Group
              </th>
              <th class="px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-400">
                Progress
              </th>
          ...
          <tbody>
            <template
              v-for="patrol in visiblePatrols"
              :key="patrol.id"
            >
              <tr
                class="border-b border-slate-50 hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer group"
              ...
              >
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2.5">
                    <div class="p-1 rounded-2xl bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/60 shadow-sm shrink-0">
                      <div
                        class="w-10 h-10 rounded-xl overflow-hidden flex items-center justify-center text-base font-black"
                        :class="patrol.status === 'active' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-400'"
                      >
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    ...
                    <div class="w-16 bg-slate-100 dark:bg-slate-950 rounded-full h-1.5 shrink-0">
                      <div
                ...
                <td class="px-4 py-3">
                  <div
                    v-if="getCurrentCp(patrol.id)"
                    class="flex items-center gap-1.5"
                  >
                    <MapPin class="w-3 h-3 text-indigo-500 shrink-0" />
                    <div>
                      <p class="text-sm font-semibold text-slate-800 dark:text-slate-200">
                        {{ getCurrentCp(patrol.id).name }}
                      </p>
                      <p class="text-xs text-slate-400">
                        {{ getCurrentCp(patrol.id).scanTime || '—' }}
                      </p>
                    </div>
                  </div>
                  <span
                ...
                <td class="px-4 py-3">
                  <div
                    v-if="getNextCp(patrol.id)"
                    class="flex items-center gap-1.5"
                  >
                    <Navigation class="w-3 h-3 text-slate-400 shrink-0" />
                    <div>
                      <p class="text-sm font-medium text-slate-600 dark:text-slate-300">
                        {{ getNextCp(patrol.id).name }}
                      </p>
                      <p class="text-xs text-slate-400">
                        {{ getNextCp(patrol.id).expectedTime || '—' }}
                      </p>
                    </div>
                  </div>
                  <span
                ...
                <td class="px-4 py-3">
                  <div class="flex flex-col gap-0.5">
                    ...
                    <span
                      v-if="patrol._displayNextTime && patrol._displayNextTime !== getDisplayTime(patrol)"
                      class="text-[11px] text-slate-400 font-bold"
                    >
                      Next: {{ patrol._displayNextTime }}
                    </span>
                    <span
                      v-else-if="!patrol._displayNextTime"
                ...
                <td class="px-4 py-3">
                  <span
                    class="text-xs font-bold px-2.5 py-1 rounded-full uppercase tracking-wide inline-flex items-center gap-1"
                    :class="{
                      ...
                      'bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-300': patrol.status === 'completed' && !patrol.missedCheckpoints,
                      'bg-indigo-50 text-indigo-600': patrol.status === 'scheduled'
                    }"
                  >
                    <span
                      v-if="patrol.status === 'active' && !patrol.missedCheckpoints"
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center justify-end gap-3">
                    <button
                      class="p-1.5 rounded-md hover:bg-indigo-50 text-indigo-400 hover:text-indigo-600 transition-colors"
                      title="Edit Patrol"
                      @click.stop="$emit('editPatrol', patrol)"
                    >
                      <Pencil class="w-4 h-4" />
                    </button>
                    <button
                      class="p-1.5 rounded-md hover:bg-rose-50 text-rose-400 hover:text-rose-600 transition-colors"
                      title="Delete Patrol"
                      @click.stop="$emit('deletePatrol', patrol)"
                    >
                    ...
                    <button
                      ...
                      @click.stop="$emit('openMap', patrol)"
                    >
                      <MapPin class="w-4 h-4" />
                    </button>
                    <ChevronDown
                      class="w-4 h-4 text-slate-400 group-hover:text-slate-600 dark:text-slate-300 transition-transform"
              ...
              <tr
                v-if="expandedRows.has(patrol.id)"
                class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 shadow-inner"
              >
                <td
                  colspan="8"
                  class="px-6 py-5"
                >
                  ...
                  <div class="flex items-start gap-0 overflow-x-auto custom-scrollbar pb-2 pt-2">
                    <template
                      v-for="(cp, idx) in getCheckpoints(patrol.id)"
                      :key="cp.checkpoint_id"
                    >
                      <!-- Step -->
                      <div class="flex flex-col items-center shrink-0 w-24 relative z-10">
                        <div
                          class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shadow-sm ring-4 ring-slate-50"
                          :class="['scanned', 'completed'].includes(cp.status) ? 'bg-emerald-500 text-white' : cp.status === 'missed' ? 'bg-rose-500 text-white' : cp.status === 'pending' && idx === getNextPendingIdx(patrol.id) ? 'bg-indigo-100 text-indigo-700 ring-indigo-50 animate-pulse' : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-200 dark:border-slate-800'"
                        >
                          <CheckCheck
                        ...
                        <p
                          class="text-xs font-bold mt-2.5 text-center truncate w-full px-1"
                          :class="['scanned', 'completed'].includes(cp.status) ? 'text-slate-700 dark:text-slate-200' : cp.status === 'missed' ? 'text-rose-700' : 'text-slate-500 dark:text-slate-400'"
                        >
                          {{ cp.name }}
                        </p>
                        <p
                      ...
                      <!-- Connector Line -->
                      <div
                        v-if="idx < getCheckpoints(patrol.id).length - 1" 
                    ...
                    <div
                      v-if="!getCheckpoints(patrol.id).length"
      ...
      >
        ...
        <p class="text-xs text-slate-300 mt-1">
          Patrols will appear here in real-time
        </p>
      </div>

      <!-- SECTION 2: Missed / Delayed Patrols -->
      <div
        v-if="missedPatrols.length"
        ...
        <table class="w-full text-left">
          <thead>
            <tr class="border-b border-rose-100">
              <th class="px-4 py-2 text-xs font-bold uppercase tracking-widest text-rose-400">
                Guard
              </th>
              <th class="px-4 py-2 text-xs font-bold uppercase tracking-widest text-rose-400">
                Zone / Checkpoint Group
          ...
          <tbody>
            <tr
              v-for="item in missedPatrols"
              :key="item.key"
              class="border-b border-rose-50 bg-rose-50/20"
            >
              <td class="px-4 py-2.5">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ item.guardName }}</span>
                </div>
              </td>
              <td class="px-4 py-2.5 text-sm text-slate-600 dark:text-slate-300">
                {{ item.zoneName }} / {{ item.routeName || 'Group' }}
              </td>
              <td class="px-4 py-2.5 text-sm font-semibold text-rose-800">
                {{ item.checkpointName }}
              </td>
              <td class="px-4 py-2.5 text-sm text-rose-600 font-semibold">
                {{ item.expectedTime }}
              </td>
              <td class="px-4 py-2.5">
      ...
      <div
        v-if="upcomingPatrols.length"
        class="border-t border-slate-200 dark:border-slate-800"
      >
        <div class="px-5 py-3 bg-slate-50 dark:bg-slate-900/50">
        ...
        <div class="px-5 py-3 flex gap-3 overflow-x-auto custom-scrollbar pb-4">
          <div
            v-for="patrol in upcomingPatrols"
            :key="patrol.id"
            class="shrink-0 w-36 border border-slate-200 dark:border-slate-800 rounded-xl p-3 bg-white dark:bg-slate-900 hover:shadow-sm transition-shadow"
          >
            ...
            <p
              ...
              :title="getGroupName(patrol)"
            >
              {{ getGroupName(patrol) }}
            </p>
            <div class="flex items-center gap-1.5 mt-2">
              <div class="w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-400 text-[10px] font-bold flex items-center justify-center">
                {{ patrol.guardName?.charAt(0) || '?' }}
              </div>
              <span class="text-xs text-slate-600 dark:text-slate-300 font-medium truncate">{{ patrol.guardName }}</span>
            </div>
          </div>
        </div>
      </div>
