<template>
  <div class="flex flex-col h-full bg-white dark:bg-slate-900 overflow-hidden">
    ...
    <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 shrink-0">
      <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2.5">
        Guards on Duty
      </h3>
      <div class="grid grid-cols-2 gap-2">
        <div class="flex items-center gap-2">
          <span class="w-2 h-2 rounded-full bg-indigo-500" />
          <span class="text-[10px] text-slate-600 dark:text-slate-300"><strong class="text-slate-900 dark:text-slate-100">{{ guardStats.onPatrol }}</strong> On Patrol</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="w-2 h-2 rounded-full bg-emerald-500" />
          <span class="text-[10px] text-slate-600 dark:text-slate-300"><strong class="text-slate-900 dark:text-slate-100">{{ guardStats.atCheckpoint }}</strong> At Checkpoint</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="w-2 h-2 rounded-full bg-amber-500" />
          <span class="text-[10px] text-slate-600 dark:text-slate-300"><strong class="text-slate-900 dark:text-slate-100">{{ guardStats.onBreak }}</strong> On Break</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="w-2 h-2 rounded-full bg-slate-300" />
          <span class="text-[10px] text-slate-600 dark:text-slate-300"><strong class="text-slate-900 dark:text-slate-100">{{ guardStats.offDuty }}</strong> Off Duty</span>
        </div>
      </div>
    </div>

    <!-- Quick Actions -->
    <div class="mt-auto px-4 py-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
      <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2.5">
        Quick Actions
      </h3>
