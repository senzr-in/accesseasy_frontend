...
const emit = defineEmits(["update:employee-data", "has-unsaved-changes"]);
...
// Add this watch to update form validity when fields change
watch(
  currentRecord,
  () => {
    isFormValid.value =
...
const deletePreviousRecord = async (index) => {
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/items/personalModule/${props.id}?fields=assignedUser.id,assignedUser.previousExperiences`,
      {
        headers: {
    ...
    const userId = data.data.assignedUser.id;

    // Create a copy of the experiences array and remove the item
    const updatedExperiences = [...previousRecords.value];
    updatedExperiences.splice(index, 1);

    // Update the user with the modified experiences array
...
const savePreviousRecord = async () => {
  try {
    const record = {
    ...
    const data = await response.json();
    const userId = data.data.assignedUser.id;
    let experiences = data.data.assignedUser.previousExperiences || [];

    // If experiences is not an array, initialize it
    ...
    if (isEditingRecord.value) {
      experiences[editingRecordIndex.value] = record;
    } else {
...
onMounted(async () => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/items/personalModule/${props.id}?fields=assignedUser.previousExperiences`,
      {
        headers: {
          Authorization: `Bearer ${authService.getToken()}`,
        },
    ...
    // Check if previousExperiences exists and is an array
    if (
      data.data.assignedUser &&
      Array.isArray(data.data.assignedUser.previousExperiences)
    ) {
      previousRecords.value = data.data.assignedUser.previousExperiences;
    } else {
      previousRecords.value = [];
    }

    originalRecords.value = JSON.parse(JSON.stringify(previousRecords.value));
  } catch (error) {
    console.error("Error fetching past experience data:", error);
