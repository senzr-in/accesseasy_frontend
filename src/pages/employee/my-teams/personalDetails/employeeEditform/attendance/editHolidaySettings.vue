...
const isSelected = (holidayId) => {
  return internalSelectedHolidayIds.value.includes(Number(holidayId));
};

const toggleHolidaySelection = (holidayId, isEnabled) => {
  const numericHolidayId = Number(holidayId);
  if (isEnabled) {
    if (!internalSelectedHolidayIds.value.includes(numericHolidayId)) {
      internalSelectedHolidayIds.value.push(numericHolidayId);
    }
  } else {
    internalSelectedHolidayIds.value = internalSelectedHolidayIds.value.filter(
      (id) => id !== numericHolidayId,
    );
  }
...
const fetchAvailableHolidays = async () => {
  ...
  try {
    ...
    const branchId = props.branchId; // This prop is used for filtering holidays by a specific branch

    let holidayUrl = `${import.meta.env.VITE_API_URL}/items/holiday?filter[tenant][_eq]=${tenantId}&fields=id,event,date,AssignHolidays`;

    // If a specific branchId is provided, filter holidays by it
    if (branchId) {
      holidayUrl += `&filter[branch][_eq]=${branchId}`;
    }

    // 1. Fetch holidays with AssignHolidays (array of branch IDs)
    const holidaysResponse = await fetch(holidayUrl, {
      headers: {
    ...
    // Collect all unique branch IDs from AssignHolidays across all fetched holidays
...
onMounted(() => {
  internalSelectedHolidayIds.value = props.modelValue.map(Number);
...
watch(
  () => props.modelValue,
  (newVal) => {
    internalSelectedHolidayIds.value = newVal.map(Number);
  },
  { deep: true },
);
...
watch(
  () => props.branchId,
  (newVal) => {
    // Trigger fetch when branchId changes, but only if it's not null/undefined
