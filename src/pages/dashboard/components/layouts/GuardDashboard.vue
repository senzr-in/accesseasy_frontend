  ...
  <div class="space-y-6 pb-12">
    ...
    <div class="bg-rose-500/10 border border-rose-500/25 rounded-3xl p-5 flex items-center justify-between gap-4 relative overflow-hidden">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h4 class="text-xs font-black text-rose-800 dark:text-rose-400 uppercase tracking-widest leading-none">
            Security Briefing
          </h4>
          <p class="text-xs font-semibold text-rose-700 dark:text-rose-300 mt-1.5 leading-relaxed">
            Watchlist monitor: no blacklist matches detected. 2 personnel currently overstayed limits.
          </p>
        </div>
      </div>
      <button 
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-900/80 dark:hover:bg-zinc-800/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800/50 dark:border-white/5 rounded-2xl p-5 shadow-sm relative overflow-hidden">
      ...
      <div class="bg-white dark:bg-slate-900/80 dark:hover:bg-zinc-800/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800/50 dark:border-white/5 rounded-2xl p-5 shadow-sm relative overflow-hidden">
        ...
        <h3 class="text-3xl font-black mt-2 text-rose-500">
          {{ stats.deniedToday }}
        </h3>
        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
          Failed gate validations
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 bg-white dark:bg-slate-900/80 dark:hover:bg-zinc-800/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800/50 dark:border-white/5 rounded-3xl overflow-hidden shadow-sm flex flex-col h-[500px]">
        ...
        <div class="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-4 custom-scrollbar bg-slate-50 dark:bg-slate-900/50 dark:bg-zinc-950/20">
          <div 
            v-for="person in visitorsInside.slice(0, 6)" 
            :key="person.id"
            class="p-4 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-slate-800/50 dark:border-white/5 rounded-2xl hover:border-indigo-500/30 transition-all flex flex-col justify-between shadow-sm cursor-pointer group"
            @click="openDrawer('person', person)"
          >
            <div class="flex gap-4">
              ...
              <div class="w-16 h-16 rounded-xl bg-indigo-500/10 flex items-center justify-center font-bold text-lg text-indigo-600 shrink-0">
                ...
                >
                <span v-else>{{ person.personName?.charAt(0).toUpperCase() || '?' }}</span>
              </div>
              <div class="min-w-0">
                <h4 class="text-xs font-bold text-slate-900 dark:text-white truncate leading-tight group-hover:text-indigo-600 transition-colors">
                  {{ person.personName }}
                </h4>
                <span class="text-[9px] text-slate-400 font-bold uppercase tracking-widest block mt-1 truncate">{{ person.company || 'Internal Staff' }}</span>
                <span class="text-[9px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest block mt-0.5 truncate">Access: {{ person.assignedAccessLevels?.accessLevelName || 'General' }}</span>
              </div>
            </div>
            
            <div class="mt-4 pt-3 border-t border-slate-100 dark:border-zinc-800/80 flex justify-between items-center text-[10px]">
              <span class="text-slate-400 font-bold uppercase tracking-widest">Dept: {{ person.personToMeet || '—' }}</span>
              <button 
                ...
                @click.stop="checkOutVisitor(person)"
              >
                Log Exit
              </button>
            </div>
          ...
          <div
            ...
            class="col-span-2 py-20 text-center text-xs font-black uppercase tracking-widest text-slate-400"
          >
            No active personnel verification records found inside.
          </div>
        </div>
      </div>

      <!-- Live scan activity log (Right 1-col) -->
      <div class="bg-white dark:bg-slate-900/80 dark:hover:bg-zinc-800/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800/50 dark:border-white/5 rounded-3xl overflow-hidden shadow-sm h-[500px] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-100 dark:divide-zinc-800">
          <div 
            ...
            :key="log.id"
            class="px-6 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:hover:bg-slate-800/50 dark:hoverdark:hover:bg-zinc-800/50 transition-colors flex justify-between items-center gap-4 text-xs"
          >
            <div class="min-w-0">
              <h4 class="font-bold text-slate-900 dark:text-white truncate">
                {{ log.name || 'Unknown Personnel' }}
              </h4>
