<template>
  <div class="space-y-6 pb-12">
    <!-- AI Briefing Banner -->
    <div class="bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-slate-500/0 dark:from-indigo-500/20 dark:via-purple-500/10 border border-slate-200/50 dark:border-white/5 rounded-3xl p-6 relative overflow-hidden backdrop-blur-xl">
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="space-y-2">
          <h2 class="text-sm font-black text-slate-900 dark:text-white leading-none tracking-tight">AI Operational Briefing</h2>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <button 
        ...
        class="flex items-center gap-3 p-4 rounded-2xl bg-white dark:bg-zinc-950 border border-slate-200/50 dark:border-white/5 shadow-sm hover:shadow-md transition-all active:scale-[0.98]"
      >
        ...
        <div class="text-left">
          <p class="text-xs font-black text-slate-900 dark:text-white leading-none">Register Walk-in</p>
      ...
      >
        ...
        <div class="text-left">
          <p class="text-xs font-black text-slate-900 dark:text-white leading-none">Search Database</p>
      ...
      >
        ...
        <div class="text-left">
          <p class="text-xs font-black leading-none">Emergency Muster</p>
      ...
      >
        ...
        <div class="text-left">
          <p class="text-xs font-black text-slate-900 dark:text-white leading-none">Badge Printer</p>
    ...
    <div class="bg-white/80 dark:bg-zinc-900/40 backdrop-blur-xl border border-slate-200/50 dark:border-white/5 rounded-3xl overflow-hidden shadow-sm">
      <div class="px-6 py-5 border-b border-slate-100 dark:border-zinc-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/50 dark:bg-zinc-900/50">
        <div>
        ...
        <!-- Layout toggle -->
        <div class="flex items-center gap-2 bg-slate-100 dark:bg-zinc-950/80 p-1 rounded-xl border border-slate-200/30 dark:border-zinc-800 shrink-0">
          <button 
            @click="layoutView = 'list'"
            :class="['px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all', layoutView === 'list' ? 'bg-white dark:bg-zinc-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600']"
          >
            List View
          </button>
          <button 
            @click="layoutView = 'kanban'"
            :class="['px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all', layoutView === 'kanban' ? 'bg-white dark:bg-zinc-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600']"
          >
            Kanban Board
          </button>
        </div>
      ...
      <!-- List view rendering -->
      <div v-if="layoutView === 'list'" class="overflow-x-auto">
        <table class="w-full text-sm text-left border-collapse">
          <thead class="text-[9px] font-black uppercase tracking-widest text-slate-500 bg-slate-50/50 dark:bg-zinc-950/50 border-b border-slate-100 dark:border-zinc-800 sticky top-0">
            <tr>
              <th class="px-6 py-4">Visitor</th>
              <th class="px-6 py-4">Host To Meet</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
            <tr 
              ...
              :key="visitor.id"
              class="hover:bg-slate-50/50 dark:hover:bg-zinc-900/50 transition-colors group cursor-pointer"
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-full bg-indigo-500/10 flex items-center justify-center font-bold text-xs text-indigo-600 dark:text-indigo-400 shrink-0">
                    <img v-if="visitor.photo" :src="getPhotoUrl(visitor.photo)" class="w-full h-full object-cover rounded-full" />
              ...
              <td class="px-6 py-4 text-right" @click.stop>
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button 
                    ...
                    class="h-8 px-3.5 rounded-xl bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 text-slate-700 dark:text-zinc-300 font-bold text-[9px] uppercase tracking-widest transition-all active:scale-95 hover:bg-slate-100"
                  >
                    Notify Host
                  </button>
                </div>
            ...
            <tr v-if="allQueue.length === 0">
              <td colspan="5" class="py-16 text-center text-xs font-black uppercase tracking-widest text-slate-400">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
        <!-- Waiting Column -->
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            <div 
              ...
              :key="visitor.id"
              class="p-4 rounded-2xl bg-white dark:bg-zinc-950 border border-slate-200/50 dark:border-white/5 hover:border-indigo-500/30 transition-all shadow-sm flex flex-col justify-between cursor-pointer"
              @click="openDrawer('visitor', visitor)"
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="text-xs font-bold text-slate-900 dark:text-white leading-tight">{{ visitor.personName }}</h4>
            ...
            <div v-if="visitorsWaiting.length === 0" class="py-12 text-center text-xs font-black uppercase tracking-widest text-slate-400">
              Queue is clear.
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            ...
            <div v-if="visitorsInside.length === 0" class="py-12 text-center text-xs font-black uppercase tracking-widest text-slate-400">
              No visitors inside.
            </div>
          </div>
        </div>
