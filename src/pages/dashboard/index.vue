    ...
    <template v-if="userRole === 'Guard'">
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm font-medium text-slate-500 mt-1">
              Welcome back, <span class="text-slate-700 dark:text-slate-300 font-bold">{{ userName }}</span> · {{ new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' }) }}
            </p>
          </div>
          <button
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <!-- Unauthorized Today -->
          <div class="bg-white dark:bg-zinc-950 rounded-2xl border border-slate-200 dark:border-zinc-800 p-5 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 left-0 w-1 h-full bg-rose-500 rounded-l-2xl" />
          ...
          <div class="bg-white dark:bg-zinc-950 rounded-2xl border border-slate-200 dark:border-zinc-800 p-5 shadow-sm relative overflow-hidden">
            ...
            <p
              ...
              class="text-3xl font-black text-slate-900 dark:text-white pl-2"
            >
              {{ guardStats.authorized + guardStats.unauthorized }}
            </p>
            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 pl-2">
              All events today
            </p>
          ...
          <!-- Auth Rate -->
          <div class="bg-white dark:bg-zinc-950 rounded-2xl border border-slate-200 dark:border-zinc-800 p-5 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 left-0 w-1 h-full bg-violet-500 rounded-l-2xl" />
            <p class="text-[10px] font-black uppercase tracking-widest text-violet-600 dark:text-violet-400 mb-2 pl-2">
              Auth Rate
            </p>
            <p
              v-if="guardStatsLoading"
        ...
        <div class="bg-white dark:bg-zinc-950 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
          <div class="px-5 py-4 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
            ...
            <button
              ...
              @click="$router.push('/dashboard/settings/logs')"
            >
              View All →
            </button>
          </div>
          ...
          <div
            v-else-if="guardRecentLogs.length === 0"
            class="flex items-center justify-center h-32 text-[10px] font-black uppercase tracking-widest text-slate-400"
          >
            No events today
          </div>
          ...
          >
            <div
              v-for="log in guardRecentLogs"
              :key="log.id"
              class="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-colors"
            >
              <div :class="['w-2 h-2 rounded-full shrink-0', log.ValidLogs === 'authorized' ? 'bg-emerald-500' : 'bg-rose-500']" />
              <div class="flex-1 min-w-0">
                <p class="text-sm font-bold text-slate-900 dark:text-white truncate">
                  {{ getEmployeeName(log) }}
                </p>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                  {{ formatGuardTime(log.date_created) }}
                </p>
              </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            class="group p-5 rounded-2xl bg-gradient-to-br from-indigo-500 to-blue-600 text-white shadow-lg shadow-indigo-500/20 hover:scale-[1.01] transition-all text-left flex items-center gap-4"
            @click="$router.push('/dashboard/authorize')"
          >
            <ShieldCheck class="w-8 h-8 opacity-90 shrink-0" />
            <div>
              <h3 class="text-base font-black mb-0.5">
                Scan & Authorize
              </h3>
              <p class="text-xs font-medium text-indigo-100">
                Open QR scanner to verify employee entry.
              </p>
            </div>
          </button>
          <button
            class="group p-5 rounded-2xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 shadow-md hover:shadow-lg transition-all text-left flex items-center gap-4"
            @click="$router.push('/dashboard/settings/logs')"
          >
            ...
            <div>
              <h3 class="text-base font-black text-slate-900 dark:text-white mb-0.5">
                Live Event Logs
              </h3>
              <p class="text-xs font-medium text-slate-500 dark:text-zinc-400">
                View all access events across entry points.
              </p>
            </div>
          </button>
        </div>
    ...
    <template v-else-if="userRole === 'Employee'">
      ...
      <div class="flex flex-col gap-6">
        <div>
          ...
          <p class="text-sm font-medium text-slate-500 mt-1">
            Here is your personal access summary for today.
          </p>
        </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <div>
            <p :class="['text-[13px] font-bold', action.labelColor]">
              {{ action.label }}
            </p>
            <p class="text-[9px] font-black text-slate-500 dark:text-zinc-500 mt-1 uppercase tracking-widest">
              {{ action.sub }}
            </p>
          </div>
        </button>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-md flex flex-col gap-3 relative overflow-hidden">
          ...
          <div class="pl-2">
            <p
              v-if="statsLoading"
              class="text-3xl font-black text-slate-200 dark:text-zinc-800 animate-pulse"
            >
              —
            </p>
            <p
              v-else
              class="text-3xl font-black text-rose-600 dark:text-rose-500"
            >
              {{ counts.unauthorized }}
            </p>
            <p class="text-[9px] font-black text-rose-400/80 uppercase tracking-widest mt-1">
              Today's Denials
            </p>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-md flex flex-col gap-3 relative overflow-hidden group">
          <div class="absolute top-0 left-0 w-1 h-full bg-slate-200 dark:bg-zinc-800 group-hover:bg-emerald-500 transition-colors" />
          <div class="flex items-center justify-between pl-2">
          ...
          <div class="pl-2">
            ...
            <p class="text-[9px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mt-1">
              Successful Entries
            </p>
          </div>
        </div>
        ...
        <!-- Devices Status -->
        <div class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-md flex flex-col gap-3 relative overflow-hidden group">
          <div class="absolute top-0 left-0 w-1 h-full bg-slate-200 dark:bg-zinc-800 group-hover:bg-blue-500 transition-colors" />
          <div class="flex items-center justify-between pl-2">
          ...
          <div class="pl-2">
            ...
            >
              ...
              <p class="text-sm font-bold text-slate-400 dark:text-zinc-600">
                / {{ counts.devices }}
              </p>
            </div>
            <p class="text-[9px] font-black text-blue-500 uppercase tracking-widest mt-1">
              100% Operational
            </p>
          </div>
        </div>
        ...
        <!-- Active Doors -->
        <div class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-md flex flex-col gap-3 relative overflow-hidden group">
          <div class="absolute top-0 left-0 w-1 h-full bg-slate-200 dark:bg-zinc-800 group-hover:bg-indigo-500 transition-colors" />
          <div class="flex items-center justify-between pl-2">
        ...
        <div class="space-y-4">
          <div class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl p-6 shadow-md flex flex-col justify-center h-[calc(100%-2.5rem)]">
            ...
            >
              ...
              <p
                v-if="authPercentage < 80"
                class="text-[10px] text-amber-600 font-bold uppercase tracking-widest flex items-center gap-1 mt-4"
              >
          ...
          <div class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-md h-[calc(100%-2.5rem)] flex flex-col">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-zinc-800 shrink-0">
              ...
              <router-link
                ...
                class="text-[9px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors flex items-center gap-1"
              >
              ...
              <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                  <tr
                    ...
                    class="h-32 bg-slate-50/50 dark:bg-zinc-900/50"
                  >
                    <td
                      colspan="3"
                  ...
                  >
                    <td class="px-5 py-3">
                      ...
                      <div class="text-[9px] text-slate-500 dark:text-zinc-500 font-black uppercase tracking-widest mt-1">
                    ...
                    <td class="px-5 py-3 text-right">
                      <span
                        ...
                        class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[9px] font-black bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 uppercase tracking-widest"
                      >
                        <CheckCircle class="w-3 h-3 border-emerald-500" /> Auth
                      </span>
                      <span
                        v-else
                        class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[9px] font-black bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20 uppercase tracking-widest"
                      >
