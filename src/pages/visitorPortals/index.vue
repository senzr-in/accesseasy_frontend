<template>
  <div :class="embedded ? 'space-y-4' : 'p-4 md:p-6 h-full overflow-y-auto space-y-4 custom-scrollbar bg-slate-50 dark:bg-slate-800/50 '">
    ...
    <div
      v-else
      class="flex items-center justify-end shrink-0 gap-3"
    >
      <select v-model="sortOrder" class="ae-input text-xs py-1.5 px-3 h-auto min-w-[140px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl outline-none cursor-pointer shadow-sm hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 items-start">
        <div
          v-for="(portal, index) in sortedPortals"
          :key="portal.id"
          class="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm hover:shadow-md animate-in fade-in slide-in-from-bottom-4 w-full transition-shadow"
        ...
        >
          ...
          <div class="flex items-center gap-3 p-4 pr-20">
            ...
            <div class="min-w-0 flex-1 flex justify-between items-center gap-2">
              ...
              <button
                class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95 shrink-0"
                title="Print QR Code"
          ...
          <div class="grid grid-cols-3 divide-x divide-slate-100  border-y border-slate-100 dark:border-slate-700 ">
            <div 
              class="py-2 text-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              @click="$router.push({ path: '/dashboard/visitors', query: { tab: 'analytics', filter: 'total', portalId: portal.id } })"
            >
              <p class="text-base font-black text-slate-900 dark:text-slate-100 leading-none">
                {{ portal.anim.total }}
              </p>
              <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
                Total
              </p>
            </div>
            <div 
              class="py-2 text-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              @click="$router.push({ path: '/dashboard/visitors', query: { tab: 'analytics', filter: 'today', portalId: portal.id } })"
            >
              <p class="text-base font-black text-emerald-500 leading-none">
                {{ portal.anim.today }}
              </p>
              <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
                Today
            ...
            <div 
              class="py-2 text-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              @click="$router.push({ path: '/dashboard/visitors', query: { tab: 'analytics', filter: 'active', portalId: portal.id } })"
            >
              <p class="text-base font-black text-blue-500 leading-none">
                {{ portal.anim.active }}
              </p>
          ...
          <div class="px-4 py-3">
            <div class="flex items-center gap-2">
              <div class="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 min-w-0">
                ...
                <a :href="portal.url" target="_blank" class="text-[9px] font-mono font-semibold text-blue-500 hover:underline truncate">{{ portal.url }}</a>
              </div>
              <button
          ...
          <div class="px-4 py-2.5 border-t border-slate-100 dark:border-slate-700  flex items-center justify-between">
            <div class="flex items-center gap-1.5">
              <button
                class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95"
                @click="openPortal(portal)"
              >
                <ExternalLink class="w-3 h-3 text-slate-400" /> Open
              </button>
              <button
                :disabled="!permissions.update"
                class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white disabled:opacity-50 transition-all active:scale-95"
                @click="editPortal(portal)"
              >
                <Pencil class="w-3 h-3 text-slate-400" /> Edit
              </button>
              ...
              <div class="relative">
                <button
                  class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95"
                  @click.stop="toggleShare(portal.id)"
                >
                  <Share2 class="w-3 h-3 text-slate-400" /> Share
                </button>
                <div 
                  v-if="activeShareMenu === portal.id" 
                  class="absolute top-[calc(100%+4px)] left-0 w-32 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xl overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-200"
                >
                  <button 
                    class="w-full flex items-center gap-2 px-2 py-2 text-[8px] font-bold uppercase tracking-wider hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white transition-colors" 
                    @click.stop="shareWhatsApp(portal); activeShareMenu = null"
                  >
                    <MessageCircle class="w-3 h-3 text-emerald-500" /> WhatsApp
                  </button>
                  <button 
                    class="w-full flex items-center gap-2 px-2 py-2 text-[8px] font-bold uppercase tracking-wider hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white transition-colors border-t border-slate-100 dark:border-slate-800" 
                    @click.stop="shareEmail(portal); activeShareMenu = null"
                  >
                    <Mail class="w-3 h-3 text-blue-500" /> Email
                  </button>
                </div>
              </div>
            </div>
...
import {
  ...
  MessageCircle, Mail, Settings, ExternalLink, Share2, Pencil,
...
const activeShareMenu = ref(null);
...
const sortedPortals = computed(() => {
  return [...portals.value].sort((a, b) => {
    switch (sortOrder.value) {
      case 'newest': return new Date(b.date_created || 0) - new Date(a.date_created || 0);
      case 'oldest': return new Date(a.date_created || 0) - new Date(b.date_created || 0);
