<template>
  <div class="p-6 md:p-8 h-full overflow-y-auto">
    ...
    <div class="flex items-start justify-between mb-8">
      <div>
        <h1 class="text-2xl font-black text-slate-900 dark:text-white mb-1">Visitor Portal</h1>
        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Your public-facing visitor check-in page.</p>
      </div>
    </div>
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-24 gap-4">
      ...
      <p class="text-sm font-semibold text-slate-400">Loading portal...</p>
    </div>
    ...
    >
      ...
      <h3 class="text-xl font-black text-slate-800 dark:text-white mb-2">No Visitor Portal Yet</h3>
    ...
    <div v-else class="space-y-6 max-w-3xl">
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="w-14 h-14 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex items-center justify-center overflow-hidden shrink-0">
              <img
                ...
                @error="logoUrl = null"
              />
              <Globe v-else class="w-7 h-7 text-slate-300 dark:text-slate-600" />
            </div>
            <div>
              <h2 class="text-lg font-black text-slate-900 dark:text-white">{{ portal.Title }}</h2>
              <p class="text-xs font-semibold text-slate-400">Visitor Check-in Portal</p>
            </div>
          </div>
        ...
        <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Portal Link</p>
          <div class="flex items-center gap-2">
        ...
        <div class="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-800">
          <div class="px-6 py-5 text-center">
            ...
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Total Visitors</p>
          </div>
          <div class="px-6 py-5 text-center">
            <p class="text-2xl font-black text-emerald-500">{{ stats.today }}</p>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Today</p>
          </div>
          <div class="px-6 py-5 text-center">
            <p class="text-2xl font-black text-blue-500">{{ stats.active }}</p>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Active Now</p>
          </div>
        </div>

        <!-- Share Footer -->
        <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between flex-wrap gap-3">
          <p class="text-xs font-semibold text-slate-400">Share portal with visitors</p>
          <div class="flex items-center gap-2">
            <button
              @click="shareWhatsApp"
    ...
    <div v-if="deleteDialog" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" @click="deleteDialog = false"></div>
      <div class="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <h3 class="text-lg font-black text-slate-900 dark:text-white mb-2 text-center">Delete Portal?</h3>
        ...
        <div class="flex gap-3">
          <button @click="deleteDialog = false" class="flex-1 py-2.5 rounded-xl text-sm font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
            Cancel
          </button>
          <button @click="deletePortal" :disabled="deleting" class="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-rose-500 hover:bg-rose-600 disabled:opacity-60 transition-colors flex items-center justify-center gap-2">
