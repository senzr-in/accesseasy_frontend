<template>
  <div :class="embedded ? 'space-y-4' : 'p-4 md:p-6 h-full overflow-y-auto space-y-4 custom-scrollbar bg-slate-50 '">
    <!-- Back Navigation Bar (hidden when embedded in dashboard) -->
    <div v-if="!embedded" class="flex items-center justify-between shrink-0 animate-in fade-in slide-in-from-top-4 duration-500">
      <button 
        @click="$router.push('/dashboard/visitors')"
        class="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 text-slate-500 hover:text-slate-900 bg-white/80 backdrop-blur-md font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-sm cursor-pointer hover:shadow-md"
      >
    ...
    <div v-else class="flex items-center justify-between shrink-0">
    ...
    <div
      ...
      class="relative overflow-hidden flex flex-col items-center justify-center py-24 rounded-3xl border border-slate-200  bg-white/40  backdrop-blur-xl text-center px-6 animate-in fade-in zoom-in-95 duration-700 shadow-xl"
    >
      <!-- Background Accents -->
      ...
      <div class="relative z-10 flex flex-col items-center">
        ...
        <p class="text-sm font-medium text-slate-500  max-w-md mb-10 leading-relaxed">
    ...
    >
      ...
      <div class="flex items-start gap-4 p-5 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50   border border-blue-100  shadow-sm animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <div>
          <h4 class="text-[11px] font-black text-slate-800  uppercase tracking-widest mb-1">
            Pro Tip
          </h4>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-3 items-start">
        <div
          ...
          :key="portal.id"
          class="group relative bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md animate-in fade-in slide-in-from-bottom-4 w-full transition-shadow"
        ...
        >
          ...
          <div class="absolute top-3 right-3 z-10">
            ...
            >
              <span class="w-1 h-1 rounded-full" :class="portal.status === 'published' ? 'bg-emerald-500' : 'bg-amber-500'"></span>
          ...
          <div class="flex items-center gap-3 p-4 pr-20">
            <!-- Logo -->
            <div class="w-10 h-10 rounded-xl border border-slate-100  bg-slate-50  flex items-center justify-center overflow-hidden shrink-0">
              <img v-if="portal.logoUrl" :src="portal.logoUrl" class="w-full h-full object-contain p-1" @error="portal.logoUrl = null">
              <Globe v-else class="w-5 h-5 text-slate-300 " />
            </div>
            <div class="min-w-0">
              <h2 class="text-sm font-black text-slate-900  tracking-tight truncate">{{ portal.Title }}</h2>
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ formatDate(portal.date_created) }}</p>
            </div>
          </div>

          <!-- Stats Row - compact -->
          <div class="grid grid-cols-3 divide-x divide-slate-100  border-y border-slate-100 ">
            <div class="py-3 text-center">
              <p class="text-lg font-black text-slate-900  leading-none">{{ portal.anim.total }}</p>
              <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-1">Total</p>
            </div>
            <div class="py-3 text-center">
              <p class="text-lg font-black text-emerald-500 leading-none">{{ portal.anim.today }}</p>
              <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-1">Today</p>
            </div>
            <div class="py-3 text-center">
              <p class="text-lg font-black text-blue-500 leading-none">{{ portal.anim.active }}</p>
              <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-1 flex items-center justify-center gap-1">
                <span class="w-1 h-1 rounded-full bg-blue-500"></span>Active
              </p>
            </div>
          </div>
          ...
          <div class="px-4 py-3">
            <div class="flex items-center gap-2">
              <div class="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 min-w-0">
                <Link class="w-3 h-3 text-blue-500 shrink-0" />
                <span class="text-[9px] font-mono font-semibold text-slate-500  truncate">{{ portal.url }}</span>
              </div>
              <button
                @click="copyUrl(portal)"
                class="flex items-center justify-center w-8 h-8 shrink-0 rounded-lg bg-slate-50 border border-slate-200 text-slate-400 hover:text-blue-500 hover:border-blue-400 transition-all active:scale-95"
              >
                <Check v-if="portal.copied" class="w-3 h-3 text-emerald-500" />
          ...
          <div class="px-4 py-2.5 border-t border-slate-100  flex items-center justify-between">
            <div class="flex items-center gap-1.5">
              <button @click="openPortal(portal)" class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-slate-500 border border-slate-200 hover:bg-slate-50 transition-all active:scale-95">
                <ExternalLink class="w-3 h-3" /> Open
              </button>
              <button :disabled="!permissions.update" @click="editPortal(portal)" class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-slate-700 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 transition-all active:scale-95">
                <Settings class="w-3 h-3" /> Edit
              </button>
              <button @click="shareWhatsApp(portal)" class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500/10 transition-all active:scale-95">
                <MessageCircle class="w-3 h-3" /> WA
              </button>
              <button @click="shareEmail(portal)" class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-blue-500 border border-blue-500/20 hover:bg-blue-500/10 transition-all active:scale-95">
                <Mail class="w-3 h-3" /> Email
              </button>
              <button @click="openQrModal(portal)" class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest text-violet-500 border border-violet-500/20 hover:bg-violet-500/10 transition-all active:scale-95">
                <QrCode class="w-3 h-3" /> Print QR
              </button>
            </div>
            <button :disabled="!permissions.delete || portal.deleting" @click="confirmDelete(portal)" class="p-1.5 rounded-lg text-slate-300  hover:text-rose-500 transition-colors disabled:opacity-40">
              <Trash2 v-if="!portal.deleting" class="w-3.5 h-3.5" />
    ...
    >
      ...
      />
      <div class="relative bg-white  border border-slate-200  rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <h3 class="text-xl font-black text-slate-900  mb-2 text-center tracking-tight">
          Delete Portal?
        </h3>
        <p class="text-sm font-medium text-slate-500 mb-8 text-center leading-relaxed">
          This will permanently remove <strong class="text-slate-800 ">{{ portalToDelete?.Title }}</strong>. All existing visitor links will instantly stop working.
        </p>
        <div class="flex gap-4">
          <button
            class="flex-1 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors active:scale-95"
            @click="deleteDialog = false"
          >
            Cancel
          </button>
    ...
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200"
    >
      <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-md" @click="qrModal.show = false" />
      <div class="relative bg-white  border border-slate-200  rounded-3xl shadow-2xl animate-in zoom-in-95 duration-300 w-full max-w-sm overflow-hidden">
        <!-- Modal Header -->
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 ">
          ...
          <button @click="qrModal.show = false" class="text-slate-400 hover:text-slate-600 :text-white transition-colors">
            <X class="w-4 h-4" />
          </button>
        </div>

        <!-- Printable QR Area -->
        <div id="qr-print-area" class="flex flex-col items-center px-8 py-8 bg-white">
          <!-- Logo + Brand -->
          <div class="flex items-center gap-2 mb-6">
            <img v-if="qrModal.logoUrl" :src="qrModal.logoUrl" class="w-8 h-8 rounded-lg object-contain" />
            <span class="text-base font-black text-slate-900 tracking-tight">{{ qrModal.title }}</span>
          </div>

          <!-- QR Code -->
          <div class="p-3 rounded-2xl border-2 border-slate-200 bg-white shadow-sm mb-5">
            <QrcodeVue
              :value="qrModal.url"
              :size="200"
              level="H"
          ...
          <!-- Instruction -->
          <p class="text-[10px] font-bold text-slate-500 text-center mb-1">Scan to check in</p>
          <p class="text-[8px] font-mono text-slate-400 text-center break-all max-w-[220px]">{{ qrModal.url }}</p>
        </div>
        ...
        <div class="px-6 py-4 border-t border-slate-100 flex gap-2">
          <button
            ...
            class="px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 bg-slate-100 hover:bg-slate-200 transition-colors"
          >
            Close
          </button>
          <button
          ...
          >
            <Loader2 v-if="qrModal.downloading" class="w-3.5 h-3.5 animate-spin" />
