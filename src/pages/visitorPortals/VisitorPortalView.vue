  ...
  if (content.value && content.value.fieldsConfig) {
    Object.keys(config).forEach(key => {
      if (content.value.fieldsConfig[key]) {
        const custom = content.value.fieldsConfig[key];
        config[key] = {
          ...config[key],
          visible: custom.visible ?? config[key].visible,
          required: custom.required ?? config[key].required,
          label: (custom.label && custom.label.trim()) ? custom.label : config[key].label,
          placeholder: (custom.placeholder && custom.placeholder.trim()) ? custom.placeholder : config[key].placeholder
        };
      }
    });
