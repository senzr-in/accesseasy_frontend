    ...
    <div class="p-6">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div
          v-for="stat in stats"
          :key="stat.label"
          class="rounded-2xl bg-slate-900 border border-slate-800/80 p-5 flex items-center gap-4"
        >
      ...
      <div v-if="activeTab === 'tenants'">
        <div class="rounded-2xl bg-slate-900 border border-slate-800/80 overflow-hidden">
          ...
          >
            <div class="h-8 w-8 rounded-full border-2 border-violet-500 border-t-transparent animate-spin" />
          </div>
      ...
      <div v-if="activeTab === 'dealers'">
        <div class="rounded-2xl bg-slate-900 border border-slate-800/80 overflow-hidden">
          <div class="flex items-center justify-between px-5 py-4 border-b border-slate-800">
            ...
            <button
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white text-xs font-bold transition-colors"
              @click="fetchDealers"
            >
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-5 py-3.5 text-sm text-slate-300">
                    {{ item.requestedBy?.assignedUser?.first_name || '—' }}
                  </td>
                  <td class="px-5 py-3.5 text-sm text-slate-400">
                    {{ item.requestedBy?.assignedUser?.email || '—' }}
                  </td>
                  <td class="px-5 py-3.5">
                    <span
                      :class="statusClass(item.dealerAccess)"
