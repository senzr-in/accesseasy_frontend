<template>
  <div class="h-full flex flex-col gap-4 overflow-hidden">
    <!-- Header (Teleported to Top Nav) -->
    <Teleport v-if="isMounted" to="#header-title-slot">
      <div class="flex items-center justify-between w-full h-full py-2">
        ...
        <button
          class="btn-primary text-xs flex items-center gap-1.5 h-8 px-3 shrink-0"
          @click="handleCreate"
        >
...
const fetchZones = async () => {
  ...
  try {
    const [zonesData, checkpointsData, patrolsData] = await Promise.all([
