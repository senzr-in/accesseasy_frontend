    ...
    <div class="flex-1 flex flex-col justify-center gap-8 max-w-lg mx-auto w-full">
      ...
      >
        <div class="text-center space-y-2">
          <p class="text-[10px] font-black text-emerald-600 dark:text-emerald-500 tracking-[0.3em] uppercase">
            Scan to enter
          </p>
          ...
          <p class="text-sm font-medium text-slate-500 dark:text-slate-400">
            Position the device QR code within the frame
          </p>
        </div>
        
        <!-- Scanner Mockup -->
        <div
          class="relative w-full aspect-square overflow-hidden rounded-xl bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-md group cursor-pointer"
          @click="simulateScan"
        >
          <div class="absolute inset-0 bg-white dark:bg-zinc-950 flex items-center justify-center">
            ...
            <p class="absolute bottom-10 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-600">
              Click to Simulate Scan
            </p>
          </div>
          
          <div class="absolute inset-0 pointer-events-none border-[12px] border-black/5 dark:border-white/5" />

          <!-- Scanning Animation -->
          <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div class="absolute left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-emerald-500 to-transparent shadow-[0_0_20px_rgba(16,185,129,0.8)] z-10 animate-[scan_2s_linear_infinite]" />
          </div>

          <!-- Corner Markers -->
          <div class="absolute inset-8 pointer-events-none">
            <div class="absolute top-0 left-0 w-8 h-8 border-t-4 border-l-4 border-emerald-500 rounded-tl-xl opacity-80" />
            <div class="absolute top-0 right-0 w-8 h-8 border-t-4 border-r-4 border-emerald-500 rounded-tr-xl opacity-80" />
            <div class="absolute bottom-0 left-0 w-8 h-8 border-b-4 border-l-4 border-emerald-500 rounded-bl-xl opacity-80" />
            <div class="absolute bottom-0 right-0 w-8 h-8 border-b-4 border-r-4 border-emerald-500 rounded-br-xl opacity-80" />
          </div>
          <div
            v-if="isVerifying"
          ...
          >
            <div class="space-y-4">
              <div class="relative flex justify-center">
                <div class="absolute inset-0 blur-xl bg-emerald-500/20 animate-pulse rounded-full w-16 h-16 mx-auto" />
                <Loader2 class="w-16 h-16 text-emerald-500 animate-spin relative" />
              </div>
              <p class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest animate-pulse mt-4">
                Verifying Access...
              </p>
      ...
      <div
        v-else
        class="flex flex-col items-center justify-center p-8 rounded-xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 shadow-md text-center space-y-6 animate-in slide-in-from-bottom-8 duration-500 relative"
      >
        <div
          class="w-20 h-20 rounded-full flex items-center justify-center -mt-16 bg-white dark:bg-zinc-950 shadow-md border-4 border-white dark:border-zinc-950"
          :class="result.success ? 'text-emerald-500' : 'text-rose-500'"
        >
          <CheckCircle2
            v-if="result.success"
            class="w-12 h-12"
        ...
        >
          <div class="flex flex-col gap-4 text-left">
            <div class="flex items-start gap-4">
              ...
              <div>
                <p class="text-[10px] sm:text-xs font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest leading-none mb-1">
                  Full Name
                </p>
