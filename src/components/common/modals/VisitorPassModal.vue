    ...
    <div class="relative flex flex-col items-center animate-in zoom-in-95 duration-200 w-full max-w-3xl">
      ...
      <div class="mt-6 flex flex-wrap justify-end gap-3 w-full print:hidden relative z-10">
        ...
        <button 
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs transition-colors shadow-sm ml-auto" 
          @click="$emit('quickAction', visitor)"
        >
        ...
        <button 
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-300 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-700 dark:text-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 font-semibold text-xs transition-colors shadow-sm ml-auto" 
          @click="$emit('quickAction', visitor)"
        >
        ...
        <button 
          class="px-4 py-2 rounded-lg font-semibold text-xs text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" 
          :class="(visitor.status === 'inactive' || visitor.status === 'active') ? '' : 'ml-auto'"
          @click="closeModal"
        >
          Close
        </button>
...
import { Loader2, Image, FileText, Printer, CheckCircle, LogOut } from 'lucide-vue-next';
...
const emit = defineEmits(['close', 'quickAction']);
