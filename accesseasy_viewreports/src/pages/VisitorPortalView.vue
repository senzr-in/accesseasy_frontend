<template>
  <div class="min-h-screen font-sans bg-white" :style="cssVars">

    <!-- ── Loading ──────────────────────────────────────────────────── -->
    <div v-if="loading" class="min-h-screen flex flex-col items-center justify-center gap-4 bg-white">
      <div class="w-10 h-10 rounded-full border-4 border-slate-100 border-t-blue-500 animate-spin"></div>
      <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">Loading Portal...</p>
    </div>

    <!-- ── Error ─────────────────────────────────────────────────────── -->
    <div v-else-if="error" class="min-h-screen flex flex-col items-center justify-center gap-6 px-6 text-center">
      <div class="w-20 h-20 rounded-full bg-rose-50 flex items-center justify-center mx-auto">
        <svg class="w-10 h-10 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
      </div>
      <h1 class="text-2xl font-black text-slate-900">Portal Not Found</h1>
      <p class="text-sm text-slate-500 max-w-xs">{{ error }}</p>
      <button @click="fetchPortal" class="px-6 py-2.5 rounded-full bg-slate-900 text-white text-sm font-bold hover:bg-slate-700 transition-colors">Retry</button>
    </div>
    ...
    <div v-else-if="portal">
      ...
      <nav class="sticky top-0 z-50 flex items-center justify-between px-6 md:px-10 py-4 bg-white/80 backdrop-blur-xl border-b border-slate-100 shadow-sm">
        <div class="flex items-center gap-3">
          <img v-if="logoUrl" :src="logoUrl" class="h-9 w-9 object-contain rounded-lg" @error="logoUrl = null" />
          <span class="font-black text-lg text-slate-900 tracking-tight">{{ portal.Title }}</span>
        </div>
        <button v-if="content.enableRegistrationForm !== false"
          @click="openModal"
      ...
      <section class="text-center px-6 py-16 md:py-24 max-w-3xl mx-auto">
        <span class="inline-block px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest mb-8"
        ...
        <!-- Banner Image -->
        <div v-if="bannerUrl" class="w-full max-w-2xl mx-auto rounded-2xl overflow-hidden shadow-xl border border-slate-100">
          <img :src="bannerUrl" class="w-full object-cover" style="max-height: 380px;" @error="bannerUrl = null" />
        </div>
      </section>

      <!-- Contact Strip -->
      <section v-if="content.enableContact && (content.contactPhone || content.contactEmail)"
        ...
        <a v-if="content.contactEmail" :href="`mailto:${content.contactEmail}`"
      ...
      <footer class="py-10 border-t border-slate-100 bg-slate-50 text-center px-6">
        <p class="text-xs font-bold text-slate-400">© {{ new Date().getFullYear() }} {{ portal.Title }}. All rights reserved.</p>
        <p class="text-[10px] text-slate-300 uppercase tracking-widest mt-1 font-bold">Powered by AccessEasy</p>
      </footer>
    </div>
    ...
    <Teleport to="body">
      <transition name="modal">
        <div v-if="modalOpen" class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
          <!-- Backdrop -->
          <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="modalOpen = false"></div>
          <!-- Panel -->
          <div class="relative w-full sm:max-w-lg bg-white sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
              <div>
                <h2 class="text-lg font-black text-slate-900">{{ content.registrationFormTitle || 'Visitor Check-in' }}</h2>
                <p class="text-xs font-medium text-slate-400 mt-0.5">{{ content.registrationFormDescription || 'Complete the form to check in.' }}</p>
              </div>
              <button @click="modalOpen = false" class="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>

            <!-- Success Screen -->
            <div v-if="submitted" class="flex flex-col items-center justify-center gap-5 py-16 px-6 text-center">
              <div class="w-20 h-20 rounded-full flex items-center justify-center"
                :style="{ backgroundColor: (content.primaryColor || '#2563eb') + '18' }">
                <svg class="w-10 h-10" :style="{ color: content.primaryColor || '#2563eb' }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
              </div>
              <div>
                <h3 class="text-2xl font-black text-slate-900 mb-2">Checked In!</h3>
                <p class="text-sm text-slate-500">Welcome, <strong>{{ visitorData.name }}</strong>. Your visit has been recorded.</p>
              </div>
              <div class="w-full bg-slate-50 rounded-2xl p-5 text-left space-y-2 text-sm">
                <div class="flex justify-between"><span class="font-bold text-slate-500">Check-in Time</span><span class="font-black text-slate-900">{{ checkInTime }}</span></div>
                <div class="flex justify-between"><span class="font-bold text-slate-500">Reason</span><span class="font-bold text-slate-700">{{ visitorData.reasonForVisit }}</span></div>
                <div v-if="visitorData.personToMeet" class="flex justify-between"><span class="font-bold text-slate-500">Meeting</span><span class="font-bold text-slate-700">{{ visitorData.personToMeet }}</span></div>
              </div>
            ...
            <!-- Form -->
            <div v-else class="overflow-y-auto flex-1 px-6 py-5 space-y-4">
              <!-- Name -->
              <div class="space-y-1.5">
                ...
                <input v-model="visitorData.name" type="text" placeholder="John Doe" autocomplete="name"
                  class="field-input" :class="{ 'border-rose-400': errors.name }" @blur="validateField('name')" />
                <p v-if="errors.name" class="text-[10px] text-rose-500 font-bold">{{ errors.name }}</p>
              </div>
              <!-- Mobile -->
              <div class="space-y-1.5">
                ...
                <input v-model="visitorData.mobile" type="tel" placeholder="+91 98765 43210" autocomplete="tel"
                  class="field-input" :class="{ 'border-rose-400': errors.mobile }" @blur="validateField('mobile')" />
                <p v-if="errors.mobile" class="text-[10px] text-rose-500 font-bold">{{ errors.mobile }}</p>
              </div>
              <!-- ID Type + Number -->
              <div class="grid grid-cols-2 gap-3">
                <div class="space-y-1.5">
                  ...
                  <select v-model="visitorData.govtIdType" class="field-input">
                ...
                <div class="space-y-1.5">
                  ...
                  <input v-model="visitorData.govtIdNumber" type="text" placeholder="XXXX-XXXX-XXXX"
                    class="field-input" :class="{ 'border-rose-400': errors.govtIdNumber }" @blur="validateField('govtIdNumber')" />
                </div>
              </div>
              <p v-if="errors.govtIdNumber" class="text-[10px] text-rose-500 font-bold -mt-2">{{ errors.govtIdNumber }}</p>
              ...
              <div class="space-y-1.5">
                ...
                <select v-model="visitorData.reasonForVisit" class="field-input" :class="{ 'border-rose-400': errors.reasonForVisit }" @change="errors.reasonForVisit = ''">
                ...
                <p v-if="errors.reasonForVisit" class="text-[10px] text-rose-500 font-bold">{{ errors.reasonForVisit }}</p>
              </div>
              ...
              <!-- Submit error -->
              <p v-if="submitError" class="text-xs text-rose-500 font-bold text-center py-1">{{ submitError }}</p>
            </div>

            <!-- Footer / Submit -->
            <div v-if="!submitted" class="px-6 pb-6 pt-4 shrink-0 border-t border-slate-100">
              <button @click="submitRegistration" :disabled="submitting"
                ...
                <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
